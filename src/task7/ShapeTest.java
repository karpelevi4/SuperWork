...
public class ShapeTest {
    public static void main(String[] args) {
        ...
        System.out.println(s1); // which version?
                                //виртуальные методы разрешаются при помощи динамического (позднего) связывания
                                //динамическая привязка использует объект для разрешения привязки
        System.out.println(s1.getArea()); // which version?
        System.out.println(s1.getPerimeter()); // which version?
        System.out.println(s1.getColor());
        System.out.println(s1.isFilled());
        /*System.out.println(s1.getRadius()); -невозможно вызвать метод getRadius() класса Circle,
                                                так как после преобразования вверх (расширения) теряется
                                                доступ к методам подкласса
         */
